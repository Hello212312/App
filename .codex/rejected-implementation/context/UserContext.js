...
import { ONBOARDING_DRAFT_KEY } from '../utils/onboarding';
...
export function UserProvider({ children }) {
    ...
    (async () => {
      try {
        ...
        if (raw) {
          ...
          if (s.user) setUserRaw({ ...DEFAULT_USER, ...s.user, locationSource: s.user.locationSource || (s.user.school ? 'school' : 'home'), locationConfirmed: s.user.locationConfirmed ?? (!s.user.school && !!s.user.location) });
  ...
  // Refresh date-dependent ranking after midnight and when returning to the app.
  useEffect(() => {
    if (!loaded) return;
    const refreshDay = () => {
