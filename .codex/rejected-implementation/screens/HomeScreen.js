...
import { ClosedBadge, CompanyLogo, DeadlineBadge, SectionHeader, Tag } from '../components';
...
export default function HomeScreen({ navigation }) {
 ...
 // Feature tour anchor: the spotlight points at the For You section
 const tourForYouRef = useTourTarget('home-for-you');
 ...
 }, [internships, isRemoteOnly, matchScoreCache, safeUser]);
 ...
 const topPickSet = useMemo(() => new Set(topPickIds || []), [topPickIds]);
  ...
  const todaysPick = useMemo(() => {
    ...
    const top = sorted.filter((i) => isRecommendable(i,safeUser)).slice(0, 8);
 ...
 .filter(({ item, daysLeft }) => daysLeft !== null && daysLeft >= 0 && daysLeft <= 14 && isRecommendable(item,safeUser))
 ...
 return d !== null && d >= 0 && d <= 30 && isRecommendable(item,safeUser);
 }).length;
 }, [internships,safeUser]);
 ...
 .filter((item) => item.id !== sourceId && !savedSet.has(item.id) && isRecommendable(item,safeUser))
 ...
 }, [viewedIds, internships, savedIds, safeUser]);
 ...
 const listHeader = useMemo(() => (
   <>
     ...
     <View ref={tourForYouRef} collapsable={false} style={[styles.section, { marginBottom: 0 }]}>
       <SectionHeader
         title={hasInterests ? 'For You (Beta)' : 'All Internships'}
