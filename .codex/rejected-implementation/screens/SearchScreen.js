...
import { ClosedBadge, CompanyLogo, DeadlineBadge, Tag } from '../components';
...
import { computeMatchScore, detectPayType, getCompetitivenessLevel, getEffectiveDaysLeft, getRecommendationTier, isItemExpired, isItemRemote, getPracticalFit } from '../utils/matching';
...
const FILTER_GROUPS = [
  ...
  },
  {
    ...
    // below map onto those, since the raw words were misleading in the UI.
    options: ['Stipend (you earn)', 'Unpaid', 'Program fee (you pay)'],
  },
  {
    label: 'Difficulty',
    key: 'competitiveness',
...
// We do NOT scan role/company/overview text, because that causes massive false-positives
...
function itemMatchesActiveFilters(item, activeFilters, stateRegex, precomputed) {
  ...
  const payFilters         = regularFilters.filter((f) =>
    f === 'Stipend (you earn)' || f === 'Unpaid' || f === 'Program fee (you pay)');
  ...
  const payOk =
    ...
    payFilters.some((f) => {
      ...
      if (f === 'Unpaid')                   return detectPayType(item).isUnpaid;
      return false;
    });
  ...
  const locationOk =
    locationFilters.length === 0 || itemIsNearMe(item, precomputed.user);
...
export default function SearchScreen({ navigation, route }) {
  const _ctx = useUser() || {};
  const user = useMemo(() => _ctx.user || {}, [_ctx.user]);
  ...
  const itemPrecomputed = useMemo(() => {
    ...
    for (const item of internships) {
      const { isPaid, isStipend, isUnpaid } = detectPayType(item);
      map[item.id] = { hasPaidTag: isPaid, hasStipendTag: isStipend, isUnpaid };
    }
    return map;
  }, [internships]);
  ...
  // them too, keeping counts and results in agreement.
  ...
  const results = useMemo(() => {
    ...
    // and "medicine" aren't adjacent in the source text, so word-AND matching
    ...
    const filtered = internships.filter((item) => {
      if (!hasQuery) {
        return itemMatchesActiveFilters(item, activeFilters, stateRegex, { ...(itemPrecomputed[item.id] || {}), nearbyStates, user });
      }
      ...
      return matchesQuery && itemMatchesActiveFilters(item, activeFilters, stateRegex, { ...(itemPrecomputed[item.id] || {}), nearbyStates, user });
    });

    const sorted = [...filtered];

    // Rank tier: 0 = open + eligible, 1 = not yet eligible for user's grade, 2 = closed.
    const rankTier = (it) => getRecommendationTier(it,user);
    ...
    return sorted;
  }, [isSearching, hasQuery, query, activeFilters, user, sortBy, internships, stateRegex, nearbyStates, itemPrecomputed, matchScoreCache]);
