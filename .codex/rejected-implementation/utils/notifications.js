...
// strong match (>=70%) and closing within 7 days, as a proactive recommendation
// rather than a reminder about something already tracked.

import * as Notifications from 'expo-notifications';
import { computeMatchBreakdown, getEffectiveDaysLeft, isRecommendable, MATCH_THRESHOLD } from './matching';
...
// At the 7-day threshold, a strong match (>=70%) gets a "why you should care"
// call-to-action instead of the generic reminder.
function getCopy(days, matchPct = null) {
  if (days === 7 && matchPct !== null && matchPct >= MATCH_THRESHOLD) {
    return { title: 'High match closing soon', body: (r, c) => `${r} at ${c} is closing in 7 days, and it has ${matchPct}% preference fit. Check the requirements before applying.` };
  }
...
const HIGH_MATCH_THRESHOLD = MATCH_THRESHOLD;
...
async function scheduleSavedReminders(savedIds, internships, user, startCount) {
  ...
  for (const item of savedItems) {
    ...
    const matchPct = breakdown && isRecommendable(item,user,{confirmedOnly:true}) ? breakdown.total : null;
...
async function scheduleDiscoveryAlerts(savedIds, internships, user, startCount) {
  ...
  const candidates = internships
    ...
    .map((item) => {
      ...
      if (matchPct === null || !isRecommendable(item,user,{confirmedOnly:true}) || matchPct < HIGH_MATCH_THRESHOLD) return null;
  ...
  for (const { item, matchPct } of candidates) {
    ...
    await Notifications.scheduleNotificationAsync({
      content: {
        ...
        body: `${item.role} at ${item.company} was just added and has ${matchPct}% preference fit. Take a look!`,
        sound: true,
        data: { internshipId: item.id },
      },
      trigger: {
 ...
 *  2. notifyHighMatch: nudges for strong-match (>=70%) internships the
