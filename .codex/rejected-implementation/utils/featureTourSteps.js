...
import { computeMatchBreakdown, isItemExpired, isRecommendable } from './matching';

const goTab = (screen) => (nav) => nav?.navigate('Main', { screen });

export function buildFeatureTourSteps({ internships = [], user = {} } = {}) {
  // Open a real listing for the Detail-screen steps. We want the tour to show
  // a program the student actually matches well with, NOT just the first one
  // in the list (that was landing on things like NASA OSTEM, which is often
  // *ineligible* for a given student and so a poor first impression). Score
  // every still-open program against the user and pick the highest eligible
  // match; fall back to the first open program, then anything.
  const openItems = internships.filter((i) => !isItemExpired(i));
  let demoItem = null;
  let demoBreakdown = null;
  let bestTotal = -1;
  for (const it of openItems) {
    const b = computeMatchBreakdown(it, user);
    // Skip programs the student is hard-ineligible for (score 0): the tour
    // should never showcase a program they can't actually apply to.
    if (!b || !isRecommendable(it,user)) continue;
    if ((b.total ?? 0) > bestTotal) {
      bestTotal = b.total ?? 0;
      demoItem = it;
      demoBreakdown = b;
    }
  ...
  const breakdown = demoBreakdown || (demoItem ? computeMatchBreakdown(demoItem, user) : null);
    ...
    ? [
        {
          ...
          fallback: {
            title: 'Your match score',
            body: 'Preference fit compares the interests and preferences you shared. Eligibility and practical limits are checked separately. The badge at the top of any listing shows the full breakdown.',
          },
        },
        {
          id: 'checklist',
    ...
    : [
        {
          ...
          targetId: null,
          title: 'Your match score',
          body: 'Every internship is scored from 0 to 100 against your grade, interests, and location. The badge at the top of any listing shows the full breakdown.',
        },
        {
          id: 'checklist',
          targetId: null,
  ...
  return [
    ...
    ...detailSteps,
    {
      id: 'for-you',
      navigate: goTab('Home'),
      targetId: 'home-for-you',
      holePadding: 10,
      title: 'For You is in beta',
      body: 'This feed suggests programs based on your profile. It is still in beta, so some suggestions can be off, and we are working to improve it. For the best results, we strongly recommend using Search with the filters.',
      fallback: {
        title: 'For You is in beta',
        body: 'The For You feed on Home suggests programs based on your profile. It is still in beta, so some suggestions can be off, and we are working to improve it. For the best results, we strongly recommend using Search with the filters.',
      },
    },
    {
      id: 'notifications',
      ...
      body: 'Turn notifications on or off entirely, or fine-tune deadline reminders, high-match alerts, and new-listing alerts individually from Profile.',
      fallback: {
        title: 'Customize your notifications',
        body: 'Head to Profile > Preferences > Notifications to turn alerts on or off, or fine-tune deadline reminders, high-match alerts, and new-listing alerts individually.',
      },
    },
    {
      id: 'deadlines',
