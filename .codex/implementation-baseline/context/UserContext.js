...
// Bump this whenever onboarding changes enough that every existing user
// (even ones with onboardingDone already true) should go through it again.
// Comparing against user.onboardingVersion in App.js is what forces that;
// OnboardingScreen stamps the current value on completion so it only
// re-triggers once per bump, not on every app open.
...
const DEFAULT_USER = {
  ...
  onboardingVersion: 0,   // stamped with ONBOARDING_VERSION on completion; a mismatch forces re-onboarding once
  referralSource: '',    // how the user heard about Interny, e.g. 'friend' | 'instagram_tiktok' | ...; '' = not set
  ...
  birthday: '',  // "YYYY-MM-DD" | '' = not set; collected via the onboarding date picker
  age: null,    // age in years, derived from birthday; powers age-restriction matching (e.g. "must be 16+")
  travelWillingness: '', // '' = not set | 'local' (stay near home) | 'state' (within my state) | 'anywhere'
  openToHousing: null,   // null = not set | true (open to dorms/residential programs) | false
  formatPreference: '',  // '' = no preference | 'remote' | 'inperson' (soft nudge in matching, not a hard filter)
  priorities: {},        // { interests, location, compensation, competitiveness } each 1–5 (3 = neutral default)
  appOpenCount: 0,
...
export function UserProvider({ children }) {
  ...
  // primary mechanism, because it always shows something and links out to the
  ...
  // Identifies the PostHog person with the same stable deviceId used for
  // Supabase activity_events, so events line up across both systems.
  useEffect(() => {
    if (!loaded) return;
    (async () => {
      const deviceId = await getDeviceId();
      const city = user.city || user.school?.city || '';
      const state = user.state || user.school?.state || '';
      posthog.identify(deviceId, {
        grade: user.grade || null,
        gpa_range: user.gpaRange || null,
        school: user.school?.name || null,
        location: city && state ? `${city}, ${state}` : (state || city || null),
        state: state || null,
        remote_only: typeof user.remoteOnly === 'boolean' ? user.remoteOnly : null,
        is_premium: !!user.premium,
      });
    })();
  // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [loaded, user.grade, user.gpaRange, user.school, user.state, user.city, user.remoteOnly, user.premium]);
  ...
  // because otherwise every internship in the catalog would alert on first launch.
  const captureNewIds = () => {
  ...
  const toggleSaved = (id) => {
    ...
    // because updaters must be pure (StrictMode double-invokes them, which would fire
  ...
  const setApplicationStatus = (id, status) => {
    if (!id || !status) return;
    posthog.capture('application_status_changed', { internshipId: id, status });
  ...
  const markViewed = (id) => {
    setViewedIds((prev) => {
      if (prev[0] === id) return prev; // already most-recent, so no state update
