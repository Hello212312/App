...
import { ClosedBadge, CompanyLogo, DeadlineBadge, Divider, MatchBreakdownModal, PremiumUpsellBanner, Tag } from '../components';
...
function matchLabel(score, ineligible) {
 if (ineligible) return 'Not eligible';
...
export default function DetailScreen({ route, navigation }) {
 ...
 const timer = setTimeout(() => markViewed(id), 350);
 ...
 const eligibility = useMemo(() => getEligibilityStatus(item, user || {}), [item, user]);
 const [showMatchBreakdown, setShowMatchBreakdown] = useState(false);

 const showEligibilityInfo = () => {
   Alert.alert(
 ...
 const { bg, text } = matchColors(matchScore);
 return (
 <TouchableOpacity
 ref={tourMatchRef}
 collapsable={false}
 style={[styles.matchBadge, { backgroundColor: bg }]}
 onPress={() => setShowMatchBreakdown(true)}
 activeOpacity={0.7}
 accessibilityRole="button"
 accessibilityLabel="Why this match score"
 >
 <Text style={[styles.matchBadgeText, { color: text }]}>{matchLabel(matchScore, ineligible)}</Text>
 <Text style={[styles.matchBadgeScore, { color: text }]}>{matchScore}%</Text>
 {ineligible && (
 <TouchableOpacity
 onPress={showEligibilityInfo}
 hitSlop={{ top: 8, bottom: 8, left: 4, right: 8 }}
 accessibilityRole="button"
 accessibilityLabel="Why am I not eligible?"
 >
 <Ionicons name="information-circle-outline" size={16} color={text} />
 </TouchableOpacity>
 )}
 </TouchableOpacity>
 );
 ...
 sub="Essay review, interview prep, and unlimited AI chat to help you land this."
 ...
 title={matchScore !== null ? matchLabel(matchScore, !eligibility.eligible) : ''}
 />
 </SafeAreaView>
 );
}
