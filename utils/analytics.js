...
import { ageFromBirthday } from './matching';
...
function profileFields(user = {}) {
  ...
  const liveAge = ageFromBirthday(user.birthday);
  return {
    ...
    user_age: liveAge != null ? liveAge : (typeof user.age === 'number' ? user.age : null),
    user_birthday: user.birthday || null,
    ...
    user_open_to_housing: typeof user.openToHousing === 'boolean' ? user.openToHousing : null,
    user_readiness: Array.isArray(user.readiness) && user.readiness.length ? user.readiness.join(', ') : null,
    user_priorities: user.priorities && Object.keys(user.priorities).length ? JSON.stringify(user.priorities) : null,
    user_premium: typeof user.premium === 'boolean' ? user.premium : null,
  };
}
