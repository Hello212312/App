...
export function TourProvider({ navRef, children }) {
  ...
  // moving — guards against mid-transition snapshots.
  ...
  const goToStep = async (i) => {
    ...
    if (navigated) {
      // Let the transition play — stack pushes animate longer than tab switches.
    ...
    if (rect) {
      ...
      // now, or the card is virtualized away) — show the centered fallback.
  ...
  const endTour = () => {
    ...
    if (onDetailRef.current) {
      // A tour step pushed the Detail screen — return to the tabs.
  ...
  useEffect(() => {
    ...
    if (prevPremiumRef.current === null) {
      // Seed on first load — existing premium users never see it retroactively.
